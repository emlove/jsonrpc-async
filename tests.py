...
from aiohttp.test_utils import setup_test_loop, teardown_test_loop, TestServer
import pep8
...
async def test_send_message(test_client):
    ...
    assert transport_error.value.args[0] == "Error calling method 'my_method': Cannot deserialize response body"
    ...
    assert transport_error.value.args[0] == "Error calling method 'my_method': HTTP 404 Not Found"
...
async def test_headers_passthrough(test_client):
    ...
    async def callback(*args, **kwargs):
        ...
        assert set(expected_headers.items()).issubset(set(kwargs['headers'].items()))
...
async def test_method_call(test_client):
    ...
    assert error.value.args[0] == "JSON-RPC spec forbids mixing arguments and keyword arguments"
...
async def test_method_nesting(test_client):
    ...
    assert await server.nest.testmethod("nest.testmethod") == True
    assert await server.nest.testmethod.some.other.method("nest.testmethod.some.other.method") == True
...
async def test_notification(test_client):
    ...
    assert await server.subtract(42, 23, _notification=True) == None
